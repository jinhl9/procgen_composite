...
if __name__ == '__main__':
    ...
    parser.add_argument("--num-eval-episode",
                        type=int,
    ...
    for W in W_saved:
        env = ProcgenGym3Env(num=1,
                             env_name=train_args_dict['env_name'],
                             use_backgrounds=False,
                             restrict_themes=True)
        successful_episode = eval(env=env,
                                  W=W,
                                  num_eval_episode=args.num_eval_episode,
                                  crop=train_args_dict['crop'],
                                  policy=train_args_dict['policy'])
